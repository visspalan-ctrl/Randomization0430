...
def test_admin_qr_panel_isolates_wechat_from_main_upload():
    ...
    html = page.text
    assert "2026-07-23-records-filter-layout-v1" in html
    ...
    assert "已啟用" in html
    assert "已停用" in html
    assert "中心 Logo" not in html
    assert "uploadQrLogo" not in html
    assert 'id="qrLogoFile"' not in html
    ...
    assert "2026-07-23-records-filter-layout-v1" in records_html
    assert "WhatsApp/Wechat 號" in records_html
    assert ">WhatsApp 號<" not in records_html
    assert 'id="recordsFilterKeyword"' in records_html
    assert "recordMatchesKeyword" in records_html
    assert 'id="recordsFilterSubjectCode"' not in records_html
    assert 'id="recordsFilterPhone"' not in records_html
    assert "編碼 / 手機號 / WhatsApp·Wechat 號 / 姓名" in records_html
...
def test_dynamic_qr_target_must_switch_after_three_consecutive():
    ...
    targets = [
        ...
        "https://wa.me/streak_b",
        "https://wa.me/streak_c",
    ]
    admin_post(
        client,
        "/admin/qr-config",
